...
class Bishop : public Piece {
    Bishop(PlayerColor color) : Piece(color, 'B') {}
    bool isValidMove(int startX, int startY, int endX, int endY, Board& board) const {
    ...
    bool threatensPosition(int x, int y, Board& board) const {
        for (int i = -7; i <= 7; ++i) {
    ...
    std::vector<std::vector<int>> generatePossibleMoves(int startX, int startY, Board& board) const {
